...
public class BookCreatureService {
    ...
    public BookCreature createBookCreatureWithoutRelations(String name,
        ...
        validateUniqueness(creature, null);
    ...
    public BookCreature updateBookCreature(BookCreature creature) {
        validateUniqueness(creature, creature.getId());
    ...
    private void validateUniqueness(BookCreature creature, Long excludeId) {
        if (creature == null) {
            return;
        }
        if (creature.getName() != null) {
            boolean nameTypeExists = bookCreatureRepo.existsByNameAndType(
                    creature.getName(), creature.getCreatureType(), excludeId);
            if (nameTypeExists) {
                throw new IllegalArgumentException("Duplicate name + creatureType");
            }
        }
        if (creature.getName() != null && creature.getCoordinates() != null
                && creature.getCoordinates().getY() != null) {
            boolean nameCoordsExists = bookCreatureRepo.existsByNameAndCoordinates(
                    creature.getName(),
                    creature.getCoordinates().getX(),
                    creature.getCoordinates().getY(),
                    excludeId);
            if (nameCoordsExists) {
