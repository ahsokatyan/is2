...
import com.antonov.is2.services.*;
...
public class BookCreatureBean implements Serializable {
    ...
    public void createCreature() {
        try {
            ...
            addMessage("Успех", "Существо создано успешно!", FacesMessage.SEVERITY_INFO);
            loadData();
            initNewCreature();
        } catch (Exception e) {
            addMessage("Ошибка", "Не удалось создать существо: " + resolveMessage(e), FacesMessage.SEVERITY_ERROR);
        }
    }
    ...
    public void updateCreature() {
        try {
            ...
            addMessage("Успех", "Существо обновлено успешно!", FacesMessage.SEVERITY_INFO);
            loadData();
            selectedCreature = null;
        } catch (Exception e) {
            addMessage("Ошибка", "Не удалось обновить существо: " + resolveMessage(e), FacesMessage.SEVERITY_ERROR);
        }
    }
    ...
    public void deleteCreature() {
        if (selectedCreature != null) {
            try {
                ...
                addMessage("Успех", "Существо удалено успешно!", FacesMessage.SEVERITY_INFO);
                loadData();
                selectedCreature = null;
            } catch (Exception e) {
                addMessage("Ошибка", "Не удалось удалить существо: " + resolveMessage(e), FacesMessage.SEVERITY_ERROR);
            }
        }
    }
...
//    }
    ...
    private void addMessage(String summary, String detail, FacesMessage.Severity severity) {
        FacesContext.getCurrentInstance().addMessage(
                null, new FacesMessage(severity, summary, detail)
        );
    }
